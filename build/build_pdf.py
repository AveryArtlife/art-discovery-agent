...
story += [Spacer(1, 2.6 * inch),
          Table([[Paragraph("Peptide Market Audit,<br/>E-Commerce Strategy<br/>and Business Proposal", S["cover_t"])]], colWidths=[CW], rowHeights=[None], style=[("LEFTPADDING", (0, 0), (-1, -1), LM)]),
          Spacer(1, 1.3 * inch),
...
# ---------------------------------------------------------------- 3 Opportunity
story += [H1("The Reserve Clinic opportunity", "3"),
 ...
 *BUL(["The incumbents are either scaled generalists adding peptides late (Hims & Hers plans NAD+, sermorelin and glutathione before end-2026), diagnostics-first platforms converging on the same funnel (Function, Superpower, Hone, Lifeforce), or budget hormone clinics with thin trust infrastructure. Nobody owns 'the trustworthy premium front door' to peptide health.",
 ...
 H2("What Reserve Clinic would sell"),
...
story += [H1("Market definition and market size", "4"),
 ...
 SRC("Analyst inference built on recalled ranges above and the financial model (07_ReserveClinic_Financial_Model.xlsx). All three definitions must be rebuilt once syndicated reports and company filings are verified."),
 SB()]
...
story += [H1("Competitive landscape", "6"),
 ...
 SRC("research/01_telehealth_companies.md and .json; labels per row in 08_ReserveClinic_Evidence_Ledger.xlsx. Ro (GLP-1 only), Eden, Henry Meds, TRT Nation, Defy, Gameday, AlphaMD, Calibrate, Limitless Male, BodyLogicMD, Fountain, Concierge MD, Mito, Found and Craft complete the 27."),
...
story += [H1("Top-performing website findings", "7"),
 ...
 T([["Pattern", "Where reported", "Effect on conversion or trust", "Reserve Clinic stance [SR]"],
...
story += [H1("UI/UX patterns and white space", "8"),
 ...
 T([["White-space opportunity", "Evidence of gap", "How Reserve Clinic fills it"],
 ...
 SRC("Analyst inference from research/01 and research/02 messaging and audit notes; 05_ReserveClinic_Website_Strategy.md sections 1–2."),
 SB()]
...
story += [H1("Product and pricing landscape", "9"),
 ...
 IMG(C + "prices.png"), SRC("03_Peptide_Product_and_Pricing_Audit.xlsx (Price Bands); research/01, research/06 (benchmarks recalled/unverified for tesamorelin and CJC/ipamorelin). Reserve Clinic band is a financial assumption."),
 H2("Regulatory classification of what is being sold"),
 T([["Classification", "Examples found in market", "Implication for Reserve Clinic"],
...
story += [H1("Marketing and claims landscape", "10"),
 ...
 T([["Channel", "Observed use", "Assessment"],
    ...
    ["Paid search", "Certified telehealth advertisers (LegitScript required for Rx-related ads)", "Gate for Reserve Clinic; apply in Phase 0"],
...
story += [H1("Celebrity-founder findings", "11"),
 ...
 H2("Lessons that shape the Reserve Clinic design"),
...
# ---------------------------------------------------------------- 12 Positioning
story += [H1("Reserve Clinic positioning", "12"),
 ...
 *BUL(["Masterbrand: Reserve Clinic. Sub-labels: Programs (clinician-led), Library (education), Standards (governance), Shop (non-Rx).", "Design system: editorial serif wordmark, humanist sans UI, ink/bone/moss/brass palette, WCAG 2.2 AA; no syringes-as-hero, no before/after, no celebrity imagery without contract.", "Voice: plain language, honest about uncertainty, specific about what is included and excluded."]),
 SB()]

# ---------------------------------------------------------------- 13 Disick
story += [H1("Scott Disick — proposed role", "13"),
 CALLOUT("Verification status: identity verified (Scott Michael Disick, b. May 26, 1983; The Kardashians on Hulu; Talentless apparel). No public evidence of any relationship with Reserve Clinic or with any peptide company was found. Everything below is a proposed structure contingent on his interest, his representatives, his Hulu and existing endorsement obligations, and counsel."),
 ...
 T([["Area", "Proposal [SR]"],
    ...
    ["Compensation scenarios (hypothetical)", "Cash retainer; equity with vesting tied to deliverables; capped royalty on net revenue; or mixed. Modeled in 07_ReserveClinic_Financial_Model.xlsx (Celebrity Economics). No terms have been offered or accepted."],
 ...
 *BUL(["Membership fees (education, navigation, clinician Q&A, lab coordination, member pricing on non-Rx).", "Program and consultation fees collected in the structure counsel prescribes (the affiliated practice may need to be merchant of record for professional fees; the MSO charges fair-market-value management fees; pharmacies bill medication or the practice does, per state law).", "Compliant non-prescription commerce; educational products; clinician-led group programs; corporate wellness limited to education and testing.", "**Flagged for counsel:** referral fees, pharmacy rebates, prescription-volume incentives, percentage-of-collections management fees in strict CPOM states, and any revenue share with prescribers or celebrities tied to prescriptions."]),
 SRC("06_ReserveClinic_Launch_Plan.md; research/05_regulatory_brief.md; financial model. Structure is analyst recommendation pending counsel."),
 SB()]
...
story += [H1("Customer experience", "16"),
 ...
 SRC("05_ReserveClinic_Website_Strategy.md sections 4–5 [SR]."),
 SB()]

# ---------------------------------------------------------------- 17 Website concept
story += [H1("Website concept", "17"),
 P("ReserveClinic.com should feel premium, confident, modern, medically credible, educational, discreet, human, transparent, easy to navigate and mobile-first. The full specification (principles, design system, 16 page specifications, 12 user flows, 8 wireframes, draft copy, technology comparison) is in 05_ReserveClinic_Website_Strategy.md; the essentials follow."),
...
/peptides, /peptides/{slug}  Library (FDA status +      /account                 Member dashboard
  evidence grade; 'offered by Reserve Clinic?')                /account/program | subscriptions | labs |
...
story += [H1("Sample wireframes", "19"),
 P("Low-fidelity wireframes for the eight key screens are in 05_ReserveClinic_Website_Strategy.md section 6. Two are reproduced here."),
 H3("Homepage — desktop (left) and mobile (right)"),
 WIRE([("Desktop 1280", 2.6, [
   (0, 0, 100, 6, ["Reserve Clinic   Programs  Library  Science  Team  Pricing            [Check eligibility]   Account"], "band"),
  ...
  ("Mobile 390", 1, [
   (0, 0, 100, 6, ["☰  Reserve Clinic     Acct"], "band"), (0, 6, 100, 4, ["Serving 12 states"], "chip"),
 ...
 *BUL(["**Build:** Next.js marketing site, library, program pages, quiz UI, member dashboard; consent-logging service; analytics pipeline with consumer-health-data filtering.", "**License:** CMS with medical-review workflow (Sanity/Contentful); Shopify for non-Rx; Stripe Billing; identity verification (Persona/Veriff/Stripe Identity); telehealth/EHR/e-Rx and provider network (candidates to diligence: OpenLoop, Wheel, SteadyMD, Healthie with DoseSpot/Surescripts); lab integration vendor; consent-management platform; Klaviyo, Twilio (TCPA consent), HIPAA-eligible messaging; Zendesk.", "**Integrate:** 503A/503B pharmacies with status webhooks and cold-chain carriers; Quest/Labcorp via lab vendor; LegitScript certification; SOC 2 vendors with BAAs.", "**Operations:** pharmacy quality agreements and recall SOP; adverse-event and pharmacovigilance SOP; cold-chain exceptions tracking; certificates of analysis for non-Rx products; cybersecurity program (pen test, incident response, cyber insurance)."]),
 SRC("05_ReserveClinic_Website_Strategy.md section 8 [SR]; vendor pricing benchmarks in research/06 (recalled/unverified)."),
 SB()]

# ---------------------------------------------------------------- 21 Clinical and pharmacy partner model
story += [H1("Clinical and pharmacy partner model", "21"),
 P("Reserve Clinic (brand and management-services organization) does not practice medicine or dispense drugs. It contracts with an affiliated professional entity that employs or contracts licensed clinicians, and with licensed pharmacies that dispense on valid patient-specific prescriptions. Counsel must confirm the structure state by state; the diagram below is the working hypothesis."),
 MONO("""  Member ──► ReserveClinic.com (brand/MSO: marketing, technology, non-clinical support, billing services)
...
story += [H1("Launch strategy", "23"),
 IMG(C + "gantt.png", maxh=2.5 * inch), SRC("06_ReserveClinic_Launch_Plan.md; timing per Base scenario [FA]."),
...
story += [H1("Channel plan", "24"),
 ...
 SRC("06_ReserveClinic_Launch_Plan.md sections 4–5 [SR/FA]."),
 SB()]

# ---------------------------------------------------------------- 25 Financial model
m12 = lambda s, k: mv["monthly"][s][k][11]
story += [H1("Financial model", "25"),
 P("The model (07_ReserveClinic_Financial_Model.xlsx) is driver-based and editable: 68 labeled assumptions in three scenario columns feed three 36-month model sheets, a scenario summary, sensitivity grids, hypothetical celebrity-economics structures and an assumptions register with rationale, source, confidence, sensitivity and validation method. Every output is a formula; no result is hard-coded. Nothing here is a forecast of results."),
 IMG(C + "scen.png"), SRC("07_ReserveClinic_Financial_Model.xlsx, Model_* sheets. Financial assumptions [FA]; benchmark-informed inputs are labeled recalled/unverified in the assumptions register."),
 ...
 SRC("07_ReserveClinic_Financial_Model.xlsx (Scenario Summary), recalculated 2026-09-02. Prescribing rate is a clinical outcome monitored for safety, not a target."),
...
story += [H1("Scenario and sensitivity analysis", "26"),
 ...
 IMG(C + "ltv.png", maxh=2.5 * inch), SRC("07_ReserveClinic_Financial_Model.xlsx (Sensitivity). Contribution = program price net of refunds and fees minus pharmacy, monitoring, shipping and support per member-month."),
...
story += [H1("Funding and resource requirements", "27"),
 ...
 SRC("06_ReserveClinic_Launch_Plan.md section 7 (plan view) and 07_ReserveClinic_Financial_Model.xlsx (model view). The two views differ by design: the plan view is a year-1 budget; the model view is cumulative burn to break-even including years 2–3 where applicable."),
 ...
 SRC("07_ReserveClinic_Financial_Model.xlsx (Celebrity Economics); licensing benchmarks recalled/unverified."),
 SB()]
...
story += [H1("Principal risks and mitigations", "29"),
 T([["Risk", "Likelihood / impact", "Mitigation"],
    ...
    ["Trademark or naming conflict: RESERVE is a widely used mark element, CLINIC is descriptive, and 'clinic' in a business name may be regulated for non-professional entities in some states", "Medium / Medium", "Clearance search in Classes 5, 35, 44, 25 before spend; counsel opinion on which entity may use the name; fallback naming"],
...
story += [H1("Recommended next steps", "31"),
 T([["Priority", "Action", "Owner", "Timing", "Why it is first"],
    ...
    ["3", "Trademark clearance and filing for RESERVE CLINIC (Classes 5, 35, 44, 25); confirm domain registrar account; secure social handles", "GM; IP counsel", "Weeks 1–3", "RESERVE-formative marks are common and 'clinic' naming may be regulated; a conflict later is expensive"],
...
story += [H1("Sources and appendices", "32"),
 P("The complete evidence register (790 entries with URLs, access dates and labels) is in 08_ReserveClinic_Evidence_Ledger.xlsx. Selected primary and high-value sources are listed below; entries marked (snippet) were seen only as search-result excerpts of the cited page."),
 ...
 T([["File", "Contents"], ["01_Peptide_Market_Landscape.csv", "109 companies by segment with legal model, business model, pricing examples, ranks, evidence confidence and source"], ["02_Competitor_Website_Audit.xlsx", "Framework, scores (with assessment status), score notes, profiles, journey and medical notes, messaging, channels, rankings, 374 sources"], ["03_Peptide_Product_and_Pricing_Audit.xlsx", "136-item product inventory, price bands, monetization patterns, claims classification, regulatory legend, pricing research plan"], ["04_Celebrity_Founder_Database.xlsx", "59 classified relationships, summary formulas, lessons, principal watchlist, sources"], ["05_ReserveClinic_Website_Strategy.md", "Principles, design system, sitemap, 16 page specs, 12 flows, 8 wireframes, draft copy, technology comparison and stack"], ["06_ReserveClinic_Launch_Plan.md", "Six phases with objectives, owners, dependencies, budgets, KPIs, risks, exit criteria; roadmaps; measurement; team; budget; pricing plan; go/no-go; crisis readiness"], ["07_ReserveClinic_Financial_Model.xlsx", "Driver-based 36-month model, three scenarios, sensitivity, celebrity economics, assumptions register"], ["08_ReserveClinic_Evidence_Ledger.xlsx", "790 evidence entries, principal verification, market size, benchmarks, compliance matrix, assumption traceability, verification backlog"]], widths=[2.3 * inch, CW - 2.3 * inch], font=7.3),
 H2("Appendix C — Verification backlog (priority 1)"),
 *BUL(["Re-run website audits from rendered pages with screenshots for the top 30 companies.", "Verify Hims & Hers, LifeMD and WW 2025–2026 filings; Function, Superpower and Lifeforce funding; syndicated market reports.", "Open FDA 503A bulks-list page and PCAC July 2026 materials; confirm any final rule.", "USPTO clearance for RESERVE CLINIC; WHOIS for reserveclinic.com; social handles.", "NPI, state board and ABIM checks for the intended physician; identify the reported Disick GLP-1 promotional post and its terms."]),
]

doc = Doc("outputs/ReserveClinic_Business_Plan_and_Proposal.pdf")
